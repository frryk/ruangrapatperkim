...
import { collection, onSnapshot, query, orderBy, doc, updateDoc, getDoc, deleteDoc, where, getDocs } from "https://www.gstatic.com/firebasejs/10.13.1/firebase-firestore.js";
...
function renderRows(){
  ...
  adminTbody.innerHTML = pageRows.map(item => {
    ...
    if (item.status === "Approved") {
      ...
      const fullLink  = `${location.origin}/${proofLink}`;
      formBuktiCell = `
        <a class="inline-flex items-center rounded-md border border-blue-300 text-blue-700 px-2.5 py-1 text-sm hover:bg-blue-50 mr-2"
           href="${proofLink}" target="_blank" rel="noopener">Buat Link Form Bukti</a><br>
        <button class="inline-flex items-center rounded-md border border-gray-300 px-2.5 py-1 text-sm hover:bg-gray-50 mt-1"
                data-action="copy" data-link="${fullLink}">Copy Link Proof</button>
      `;
    }
